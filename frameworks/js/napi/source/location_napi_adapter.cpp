...
CallbackManager<BluetoothScanResultCallbackNapi> g_bluetoothSearchCallbackHosts;
    ...
    std::unique_ptr<LocatingRequiredDataConfig>& config, sptr<LocatingRequiredDataCallbackNapi> callback)
{
    ...
    asyncContext->callbackHost_ = callback;
    SetWlanMatchExecuteFunc(asyncContext);
    asyncContext->completeFunc = [&](void* data) -> void {
        if (data == nullptr) {
            return;
        }
...
{
    return DoWlanMatchRequest<SingleScanAsyncContext>(env, info, CreateSingleWifiScanAsyncContext);
}

FindMatchingWlanAsyncContext* CreateFindMatchingWlanAsyncContext(const napi_env& env,
    std::unique_ptr<LocatingRequiredDataConfig>& config, sptr<LocatingRequiredDataCallbackNapi> callback)
{
    ...
    asyncContext->completeFunc = [&](void* data) -> void {
        ...
        for (size_t i = 0; i < context->matchingWlanInfos_.size(); ++i) {
            const auto& info = context->matchingWlanInfos_[i];
            napi_value object;
        ...
        context->result[PARAM1] = arrayResult;
        if (context->callbackHost_) {
            context->callbackHost_ = nullptr;
        }
    };
...
napi_value RemoveGnssGeofence(napi_env env, napi_callback_info info)
{
    ...
    if (asyncContext->callbackHost_ == nullptr) {
        auto locationGnssGeofenceCallbackHost =
            sptr<LocationGnssGeofenceCallbackNapi>(new LocationGnssGeofenceCallbackNapi());
...
void SetCompleteFuncForRemoveGnssGeofenceContext(GnssGeofenceAsyncContext* asyncContext)
{
    asyncContext->completeFunc = [&](void* data) -> void {
        ...
        if (callbackHost != nullptr && context->errCode == ERRCODE_SUCCESS &&
            ...
            LocationErrCode errCode = callbackHost->DealGeofenceOperationResult();
            if (errCode == ERRCODE_SUCCESS) {
                RemoveCallbackToGnssGeofenceCallbackHostMap(context->env, context->fenceId_);
            } else {
                context->errCode = errCode;
            }
        }
...
void RemoveCallbackToGnssGeofenceCallbackHostMap(napi_env env, int fenceId)
{
    ...
    if (iterForDelete != g_gnssGeofenceCallbackHostMap.end()) {
        if (iterForDelete->second != nullptr && env == iterForDelete->second->GetEnv()) {
            iterForDelete->second->DeleteHandler();
        } else {
            LBSLOGE(LOCATOR_STANDARD, "env is different when DeleteHandler");
        }
    ...
    asyncContext->completeFunc = [&](void* data) -> void {
        ...
        if (context->errCode != ERRCODE_SUCCESS) {
            ...
            if (callbackNapi != nullptr) {
                ...
                context->handlerRef = nullptr;
            }
        }
    };
}

static bool CheckBluetoothSearchParams(napi_env env, napi_value paramsObj)
{
    napi_valuetype valueType;
    napi_typeof(env, paramsObj, &valueType);
    if (valueType != napi_object) {
...
static bool CheckLocationSwitchEnabled(napi_env env)
{
    ...
    if (!isLocationEnabled) {
        ThrowBusinessError(env, ERRCODE_SWITCH_OFF);
        return false;
    }
    return true;
}
...
static bool CheckAndCreateBluetoothSearchContext(napi_env env, napi_value* argv,
    BluetoothSearchAsyncContext*& asyncContext, sptr<BluetoothScanResultCallbackNapi>& callback)
{
    if (napi_create_string_latin1(env, "StartBluetoothSearch",
        NAPI_AUTO_LENGTH, &asyncContext->resourceName) != napi_ok) {
        delete asyncContext;
        GET_AND_THROW_LAST_ERROR(env);
        return false;
    ...
    if (!CreateBluetoothSearchCallback(env, argv[PARAM1], asyncContext, callback)) {
...
napi_value StartBluetoothSearch(napi_env env, napi_callback_info info)
{
    ...
    if (g_bluetoothSearchCallbackHosts.GetCallbackPtr(env, argv[PARAM1]) != nullptr) {
        LBSLOGE(LOCATOR_STANDARD, "StartBluetoothSearch: callback already exists");
        return UndefinedNapiValue(env);
    }
    ...
    g_bluetoothSearchCallbackHosts.AddCallback(env, asyncContext->handlerRef, bluetoothScanResultCallback);
    delete asyncContext;
    return UndefinedNapiValue(env);
}
...
napi_value StopBluetoothSearch(napi_env env, napi_callback_info info)
{
    ...
    LBSLOGI(LOCATOR_STANDARD, "StopBluetoothSearch: enter");
    NAPI_CALL(env, napi_get_cb_info(env, info, &argc, argv, &thisVar, &data));
    NAPI_ASSERT(env, g_locatorClient != nullptr, "locator instance is null.");

    if (argc < PARAM1) {
        LBSLOGE(LOCATOR_STANDARD, "StopBluetoothSearch: no callback provided");
        ThrowBusinessError(env, ERRCODE_SERVICE_UNAVAILABLE);
        return UndefinedNapiValue(env);
    }
    ...
    LBSLOGI(LOCATOR_STANDARD, "StopBluetoothSearch: before call g_locatorClient->StopBluetoothSearch");
    ...
    if (errCode == ERRCODE_SUCCESS) {
        g_bluetoothSearchCallbackHosts.DeleteCallbackByRef(env, bluetoothScanResultCallbackHost->GetHandleCb());
        bluetoothScanResultCallbackHost->DeleteHandler();
    }
