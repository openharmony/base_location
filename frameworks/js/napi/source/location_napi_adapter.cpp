    ...
    namespace Location
    {
        ...
        napi_value GetAddressesFromLocation(napi_env env, napi_callback_info info)
        {
            ...
            asyncContext->errCode =
                JsObjToReverseGeoCodeRequest(env, argv[0], asyncContext->reverseGeoCodeRequest) ? SUCCESS : INPUT_PARAMS_ERROR;
        ...
        napi_value GetAddressesFromLocationName(napi_env env, napi_callback_info info)
        {
            ...
            asyncContext->errCode = JsObjToGeoCodeRequest(env, argv[0], asyncContext->geoCodeRequest) ? SUCCESS : INPUT_PARAMS_ERROR;
            asyncContext->executeFunc = [&](void *data) -> void
            {
        ...
        napi_value GetIsoCountryCode(napi_env env, napi_callback_info info)
        {
            ...
            {
                GetIsoCountryCodeContext *context = static_cast<GetIsoCountryCodeContext *>(data);
                napi_create_string_utf8(context->env, context->countryCode.c_str(), context->countryCode.size(), &context->result[PARAM1]);
                LBSLOGI(LOCATOR_STANDARD, "Push IsLocationEnabled result to client");
            };
        ...
        void ParameterConfiguration(napi_env env, uint32_t arrayLength, napi_value object, SetMockedLocationsAsyncContext *asyncContext)
        {
