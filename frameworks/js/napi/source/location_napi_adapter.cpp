...
napi_value SetLocationPrivacyConfirmStatus(napi_env env, napi_callback_info info)
{
    ...
    asyncContext->executeFunc = [&](void* data) -> void {
        auto context = static_cast<PrivacyAsyncContext*>(data);
        context->isSuccess = g_locatorClient->SetLocationPrivacyConfirmStatus(context->type, context->isConfirmed);
        context->errCode = (context->isSuccess) ? SUCCESS : PRIVACY_CONFIRM_STATUS_ERROR;
    };
