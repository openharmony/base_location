...
const int32_t UNKNOWN_SERVICE_ID = -1;
...
HWTEST_F(LocatorServiceTest, locatorServiceStartAndStopLocating001, TestSize.Level1)
{
    auto locatorAbility =
        sptr<LocatorAbility>(new (std::nothrow) LocatorAbility());
    auto requestConfig = std::make_unique<RequestConfig>();
    ...
    locatorAbility->StartLocating(requestConfig, callbackStub_, identity);

    locatorAbility->StopLocating(callbackStub_);
}
...
HWTEST_F(LocatorServiceTest, locatorServiceCacheLocation001, TestSize.Level1)
{
    ...
    locationNew->SetLatitude(1.0);
    locationNew->SetLongitude(2.0);
    locatorAbility->ReportLocation(locationNew, GNSS_ABILITY);
    MessageParcel reply2;
    EXPECT_EQ(REPLY_CODE_NO_EXCEPTION, locatorAbility->GetCacheLocation(reply2, identity));
}

HWTEST_F(LocatorServiceTest, locatorServiceCacheLocation002, TestSize.Level1)
{
    auto locatorAbility =
        sptr<LocatorAbility>(new (std::nothrow) LocatorAbility());
    pid_t callingPid = IPCSkeleton::GetCallingPid();
...
HWTEST_F(LocatorServiceTest, locatorServiceIsoCountryCode001, TestSize.Level1)
{
    auto locatorAbility =
        sptr<LocatorAbility>(new (std::nothrow) LocatorAbility());
    EXPECT_NE(nullptr, locatorAbility->GetIsoCountryCode());
}
