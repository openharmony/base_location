...
#include "passiveabilityproxy_fuzzer.h"
...
#ifdef FEATURE_PASSIVE_SUPPORT
bool PassiveAbility001FuzzTest(const uint8_t* data, size_t size)
{
    int index = 0;
    sptr<OHOS::Location::PassiveAbility> ability = new (std::nothrow) PassiveAbility();
    sptr<OHOS::Location::PassiveAbilityProxy> proxy =
        new (std::nothrow) PassiveAbilityProxy(ability);
...
#ifdef FEATURE_PASSIVE_SUPPORT
    OHOS::PassiveAbility001FuzzTest(data, size);
#endif
    return 0;
}
