...
constexpr int32_t FENCE_MAX_ID = 29999;
...
{
    return IsMockEnabled() || GetRequestNum() != 0 || IsMockProcessing() || IsGnssfenceRequestExist();
}

bool GnssAbility::IsGnssfenceRequestExist()
{
    std::unique_lock<ffrt::mutex> lock(gnssGeofenceRequestListMutex_);
    return gnssGeofenceRequestList_.size() != 0;
}
...
void GnssAbility::PreRestoreGeofenceRequest()
{
    if (gnssHandler_ != nullptr) {
        AppExecFwk::InnerEvent::Pointer event = AppExecFwk::InnerEvent::Get(
            static_cast<uint32_t>(GnssAbilityInterfaceCode::RESTORE_GEOFENCE_REQUEST), 0);
        gnssHandler_->SendEvent(event);
    }
}

void GnssAbility::RestoreGeofenceRequest()
{
#ifdef HDF_DRIVERS_INTERFACE_GEOFENCE_ENABLE
    // 读取固化数据
    ...
    for (auto iter : requestList) {
        ...
        if (iter->GetFenceId() > fenceId) {
            fenceId = iter->GetFenceId();
        }
    }
    if (fenceId > 0) {
        // 初始化fenceId
        InitGeofenceId(fenceId);
    }
    LBSLOGI(GNSS, "After RestoreGeofenceRequest size %{public}zu", gnssGeofenceRequestList_.size());
#endif
}
...
LocationErrCode GnssAbility::ValidateGeofenceRequest(std::shared_ptr<GeofenceRequest>& request)
{
...
int GnssAbility::ProcessFenceId(std::shared_ptr<GeofenceRequest>& request)
{
    if (request->GetFenceId() > 0) {
        return request->GetFenceId();
    }
...
LocationErrCode GnssAbility::AddGnssGeofenceToHdi(
    std::shared_ptr<GeofenceRequest>& request, int fenceId, uint32_t monitorEvent)
{
#ifdef HDF_DRIVERS_INTERFACE_GEOFENCE_ENABLE
    int32_t ret = -1;
    if (request == nullptr) {
    ...
    if (geofenceInterfaceV3 != nullptr) {
        return AddGeofenceV3(request, fenceId, monitorEvent);
    } else {
        return AddGeofenceV2(request, fenceId, monitorEvent);
    }
#else
    return ERRCODE_SUCCESS;
#endif
...
LocationErrCode GnssAbility::AddGeofenceV3(
    std::shared_ptr<GeofenceRequest>& request, int fenceId, uint32_t monitorEvent)
{
#ifdef HDF_DRIVERS_INTERFACE_GEOFENCE_ENABLE
    if (request == nullptr) {
    ...
    sptr<HDI::Location::Geofence::V3_0::IGeofenceInterface> geofenceInterfaceV3 =
        HDI::Location::Geofence::V3_0::IGeofenceInterface::Get();
    if (geofenceInterfaceV3 == nullptr) {
        LBSLOGE(GNSS, "geofenceInterfaceV3 is nullptr");
        return ERRCODE_SERVICE_UNAVAILABLE;
    }
    auto geofence = request->GetGeofence();
    HDI::Location::Geofence::V3_0::GeofenceInfo fenceInfoV3;
    fenceInfoV3.fenceIndex = fenceId;
    fenceInfoV3.latitude = geofence.latitude;
    fenceInfoV3.longitude = geofence.longitude;
    fenceInfoV3.radius = geofence.radius;
    
    int32_t ret = geofenceInterfaceV3->AddGnssGeofence(fenceInfoV3, monitorEvent);
    LBSLOGI(GNSS, "AddGnssGeofence V3.0, ret:%{public}d,fenceId:%{public}s",
        ret, std::to_string(fenceId).c_str());
    
    return (ret == 0) ? ERRCODE_SUCCESS : ERRCODE_SERVICE_UNAVAILABLE;
#else
    return ERRCODE_SUCCESS;
#endif
}
 
LocationErrCode GnssAbility::AddGeofenceV2(
    std::shared_ptr<GeofenceRequest>& request, int fenceId, uint32_t monitorEvent)
{
#ifdef HDF_DRIVERS_INTERFACE_GEOFENCE_ENABLE
    if (request == nullptr) {
    ...
    sptr<HDI::Location::Geofence::V2_0::IGeofenceInterface> geofenceInterface =
        HDI::Location::Geofence::V2_0::IGeofenceInterface::Get();
    if (geofenceInterface == nullptr) {
        LBSLOGE(GNSS, "geofenceInterface is nullptr");
        return ERRCODE_SERVICE_UNAVAILABLE;
    }
    auto geofence = request->GetGeofence();
    HDI::Location::Geofence::V2_0::GeofenceInfo fenceInfoV2;
    fenceInfoV2.fenceIndex = fenceId;
    fenceInfoV2.latitude = geofence.latitude;
    fenceInfoV2.longitude = geofence.longitude;
    fenceInfoV2.radius = geofence.radius;
    
    int32_t ret = geofenceInterface->AddGnssGeofence(fenceInfoV2, monitorEvent);
    LBSLOGI(GNSS, "AddGnssGeofence V2.0, ret:%{public}d,fenceId:%{public}s",
        ret, std::to_string(fenceId).c_str());
...
{
    LocationErrCode validationResult = ValidateGeofenceRequest(request);
    if (validationResult != ERRCODE_SUCCESS) {
    ...
    int fenceId = ProcessFenceId(request);
    if (fenceId <= 0) {
        return LOCATION_ERRCODE_INVALID_PARAM;
    }
    auto geofence = request->GetGeofence();
    request->SetRequestExpirationTimeStamp(CommonUtils::GetCurrentTimeMilSec() + geofence.expiration);
    uint32_t monitorEvent = static_cast<int>(GeofenceTransitionEvent::GEOFENCE_TRANSITION_EVENT_ENTER) |
        static_cast<int>(GeofenceTransitionEvent::GEOFENCE_TRANSITION_EVENT_EXIT);
    LocationErrCode hdiResult = AddGnssGeofenceToHdi(request, fenceId, monitorEvent);
    if (hdiResult != ERRCODE_SUCCESS) {
        return hdiResult;
    }
...
LocationErrCode GnssAbility::RemoveFence(std::shared_ptr<GeofenceRequest>& request)
{
    ...
    std::vector<std::shared_ptr<GeofenceRequest>> requestList = GetGnssGeofenceRequestList(request);
...
bool GnssAbility::SaveFenceWantAgentInfo(std::shared_ptr<GeofenceRequest> &request)
{
    if (request == nullptr) {
        LBSLOGE(GNSS, "request is invalid");
        return false;
    }
    if (request->GetWantAgent() == nullptr) {
        LBSLOGE(GNSS, "wantAgen is invalid.");
        return false;
    }
    std::unique_lock<ffrt::mutex> lock(gnssGeofenceRequestListMutex_);
...
void GnssAbility::InitGeofenceId(int32_t fenceId)
{
    LBSLOGI(GNSS, "InitGeofenceId fenceId : %{public}d", fenceId);
    fenceId_ = fenceId;
}
...
LocationErrCode GnssAbility::RemoveGnssGeofence(std::shared_ptr<GeofenceRequest>& request)
{
    ...
    return HandleDeleteGnssGeofence(request);
}

LocationErrCode GnssAbility::HandleDeleteGnssGeofence(std::shared_ptr<GeofenceRequest>& request)
{
#ifdef HDF_DRIVERS_INTERFACE_GEOFENCE_ENABLE
    int32_t ret = -1;
    if (isGeofenceHdiV3_) {
        ...
        ret = geofenceInterfaceV3->DeleteGnssGeofence(request->GetFenceId());
        LBSLOGI(GNSS, "DeleteGnssGeofence V3.0, ret:%{public}d,fenceId:%{public}s",
        ...
        ret = geofenceInterface->DeleteGnssGeofence(request->GetFenceId());
        LBSLOGI(GNSS, "DeleteGnssGeofence V2.0, ret:%{public}d,fenceId:%{public}s",
            ret, std::to_string(request->GetFenceId()).c_str());
    }
#endif
...
bool GnssAbility::UnregisterGnssGeofenceCallback(int fenceId)
{
    for (auto iter = gnssGeofenceRequestList_.begin(); iter != gnssGeofenceRequestList_.end();) {
        ...
        if (fenceId == gnssGeofenceRequest->GetFenceId()) {
            if (gnssGeofenceRequest->GetGeofenceTransitionCallback() != nullptr &&
...
bool GnssAbility::IsDuplicateAddRequestForGnssGeofence(std::shared_ptr<GeofenceRequest>& request)
{
    ...
    for (auto iter = gnssGeofenceRequestList_.begin();
        iter != gnssGeofenceRequestList_.end(); iter++) {
        auto gnssGeofenceRequest = *iter;
        if (gnssGeofenceRequest == nullptr) {
            continue;
        }
        if (gnssGeofenceRequest->GetBundleName().compare(request->GetBundleName()) == 0 &&
            ...
            if (callback != nullptr) {
                ...
                gnssGeofenceRequest->SetPid(request->GetPid());
                gnssGeofenceRequest->SetTokenId(request->GetTokenId());
                gnssGeofenceRequest->SetTokenIdEx(request->GetTokenIdEx());
                gnssGeofenceRequest->SetFirstTokenId(request->GetFirstTokenId());
            }
            return true;
        }
    }
...
bool GnssAbility::IsDuplicateOnRequestForGnssGeofence(std::shared_ptr<GeofenceRequest>& request)
{
    ...
    for (auto iter = gnssGeofenceRequestList_.begin();
        ...
        auto wantAgentPtr = gnssGeofenceRequest->GetWantAgent();
...
std::vector<std::shared_ptr<GeofenceRequest>> GnssAbility::GetGnssGeofenceRequestList(
    std::shared_ptr<GeofenceRequest>& request)
{
    std::vector<std::shared_ptr<GeofenceRequest>> matchedRequests;
    std::unique_lock<ffrt::mutex> lock(gnssGeofenceRequestListMutex_);
...
bool GnssAbility::IsSameGeofence(const GeoFence& geoFence, const GeoFence& requestGeoFence)
{
    return (geoFence.latitude == requestGeoFence.latitude) &&
           (geoFence.longitude == requestGeoFence.longitude) &&
           (geoFence.radius == requestGeoFence.radius) &&
           (geoFence.expiration == requestGeoFence.expiration) &&
           (geoFence.coordinateSystemType == requestGeoFence.coordinateSystemType);
}
...
size_t GnssAbility::GetGnssGeofenceRequestMapSize()
{
    std::unique_lock<ffrt::mutex> lock(gnssGeofenceRequestListMutex_);
...
void GnssAbility::ReducedGeoFencesCount(const std::string& bundleName)
{
    ...
    if (it != gnssGeofenceRequestCountMap_.end()) {
        ...
        if (it->second == 0) {
            gnssGeofenceRequestCountMap_.erase(it);
        }
    }
}

#ifdef HDF_DRIVERS_INTERFACE_GEOFENCE_ENABLE
void GnssAbility::ReportGeofenceOperationResult(int fenceId, int type, int result)
{
    std::unique_lock<ffrt::mutex> lock(gnssGeofenceRequestListMutex_);
    ...
    if (type == static_cast<int>(HDI::Location::Geofence::V2_0::TYPE_DELETE) &&
        result == static_cast<int>(HDI::Location::Geofence::V2_0::GEOFENCE_OPERATION_SUCCESS)) {
    ...
    bool appAliveStatus = IsProcessRunning(geofenceRequest->GetPid(), geofenceRequest->GetTokenId());
    if (!appAliveStatus) {
    ...
    gnssGeofenceCallback->OnReportOperationResult(fenceId, type, result);
}
...
std::vector<std::shared_ptr<GeofenceRequest>> GnssAbility::ReadGeoFenceRequestFromFile()
{
    ...
    cJSON *item = NULL;
    cJSON_ArrayForEach(item, geofenceRequestRoot) {
...
{
    if (!CommonUtils::IsExistFile(GEOFENCE_REQUEST_FILE_PATH)) {
        CommonUtils::CreateFile(GEOFENCE_REQUEST_FILE_PATH, "0");
    }
    std::ofstream fs(GEOFENCE_REQUEST_FILE_PATH, std::ofstream::out | std::ofstream::trunc);
    if (!fs || !fs.is_open()) {
        LBSLOGE(GNSS, "fs.is_open false, return");
        return;
    }
    ...
    for (auto gnssGeofenceRequest : gnssGeofenceRequestList_) {
        ...
        cJSON_AddItemToObject(root, std::to_string(gnssGeofenceRequest->GetFenceId()).c_str(),
            cJSON_CreateString(geoFenceRequestStr.c_str()));
    }
    auto jsonString = cJSON_Print(root);
    if (jsonString != nullptr) {
        fs.write(jsonString, strlen(jsonString));
...
#ifdef HDF_DRIVERS_INTERFACE_GEOFENCE_ENABLE
void GnssAbility::ReportGeofenceEvent(int fenceIndex, int event)
{
    std::unique_lock<ffrt::mutex> lock(gnssGeofenceRequestListMutex_);
    ...
    if (CommonUtils::GetCurrentTimeMilSec() > request->GetRequestExpirationTimeStamp()) {
        ...
        if (gnssHandler_ != nullptr) {
            AppExecFwk::InnerEvent::Pointer handlerEvent = AppExecFwk::InnerEvent::Get(
                static_cast<uint32_t>(GnssAbilityInterfaceCode::REMOVE_GEOFENCE), request);
            gnssHandler_->SendEvent(handlerEvent);
        }
        return;
    }
...
bool GnssAbility::NotifyGnssfenceStatusByWantAgent(const std::shared_ptr<GeofenceRequest> &request, int event)
{
    ...
    if (!want) {
        LBSLOGE(GNSS, "%{public}s: nullptr want.", __func__);
        return false;
    }
    want->SetParam("transition", static_cast<GeofenceTransitionEvent>(event));
    LBSLOGI(GNSS, "%{public}s: want: %{public}s", __func__, want->ToString().c_str());
    AbilityRuntime::WantAgent::TriggerInfo triggerInfo("", nullptr, want, WANT_CODE_ELEVEN);
    sptr<AbilityRuntime::WantAgent::CompletedDispatcher> data;
    AbilityRuntime::WantAgent::WantAgentHelper::TriggerWantAgent(wantAgent, nullptr, triggerInfo, data, nullptr);
    return true;
}
...
std::shared_ptr<GeofenceRequest> GnssAbility::GetGeofenceRequestByFenceId(int fenceId)
{
    for (auto iter = gnssGeofenceRequestList_.begin(); iter != gnssGeofenceRequestList_.end(); iter++) {
        ...
        if (gnssGeofenceRequest->GetFenceId() == fenceId) {
            return gnssGeofenceRequest;
        }
    }
    ...
    GnssInterfaceCode code, std::shared_ptr<GeofenceRequest>& request)
{
    ...
    lock.lock();
    if (IsGeofenceHdiV3Supported()) {
        fenceStruct.callbackV3 = geofenceCallbackV3_;
    } else {
...
#ifdef HDF_DRIVERS_INTERFACE_GEOFENCE_ENABLE
bool GnssAbility::IsGeofenceHdiV3Supported()
{
    sptr<HDI::Location::Geofence::V3_0::IGeofenceInterface> geofenceInterfaceV3 =
        HDI::Location::Geofence::V3_0::IGeofenceInterface::Get();
    if (geofenceInterfaceV3 != nullptr) {
        LBSLOGI(GNSS, "Geofence HDI V3.0 is supported");
        return true;
    }
    LBSLOGI(GNSS, "Geofence HDI V3.0 is not supported, fallback to V2.0");
    return false;
}
 
void GnssAbility::SetGeofenceHdiVersion(bool isV3)
{
    isGeofenceHdiV3_ = isV3;
    LBSLOGI(GNSS, "SetGeofenceHdiVersion isV3:%{public}d", isV3);
}
...
bool GnssAbility::RemoveHdi()
{
    ...
    geofenceCallbackV3_ = nullptr;
    isGeofenceHdiV3_ = false;
#endif
#ifdef HDF_DRIVERS_INTERFACE_GEOFENCE_ENABLE
...
void GnssHandler::InitGnssEventProcessMap()
{
    ...
    gnssEventProcessMap_[static_cast<uint32_t>(GnssAbilityInterfaceCode::RESTORE_GEOFENCE_REQUEST)] =
        [this](const AppExecFwk::InnerEvent::Pointer& event) { HandleRestoreGeofenceRequest(event); };
}
...
void GnssHandler::HandleRestoreGeofenceRequest(const AppExecFwk::InnerEvent::Pointer& event)
{
    GnssAbility::GetInstance()->RestoreGeofenceRequest();
}
