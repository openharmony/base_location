...
SelfRequestManager::SelfRequestManager()
{
    ...
    locationSwitch_ = (LocationDataRdbManager::QuerySwitchState() == ENABLED);
}
...
void SelfRequestManager::ProcessStartSelfRequestEvent()
{
    std::unique_lock<std::mutex> lock(locatorMutex_, std::defer_lock);
    lock.lock();
    if (isLocating_ || !locationSwitch_) {
...
void SelfRequestManagerHandler::ProcessEvent(const AppExecFwk::InnerEvent::Pointer& event)
{
    ...
    switch (eventId) {
        case EVENT_STARTLOCATING: {
            SelfRequestManager->ProcessStartSelfRequestEvent();
            break;
        }
