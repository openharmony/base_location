...
ErrCode LocatorAbility::IsLocationPrivacyConfirmed(int32_t type, bool& state)
{
    ...
    int userId = CommonUtils::GetUserIdByUid(identity.GetUid());
    return LocationConfigManager::GetInstance()->GetPrivacyTypeStateForUser(type, state, userId);
}
...
ErrCode LocatorAbility::AddFence(const GeofenceRequest& request)
{
    ...
    fenceRequest->SetPid(identity.GetPid());
    fenceRequest->SetTokenId(identity.GetTokenId());
    fenceRequest->SetTokenIdEx(identity.GetTokenIdEx());
    fenceRequest->SetFirstTokenId(identity.GetFirstTokenId());
